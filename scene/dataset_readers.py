...
from scene.linear_model import BasicPointCloud
...
def readNerfSyntheticInfo(
    path, white_background, eval, extension=".png", fixed_init=False
):
...
def readMultiScaleNerfSyntheticInfo(path, white_background, eval, load_allres=False):
    ...
    train_cam_infos = readMultiScale(
        path, white_background, "train", only_highres=(not load_allres)
    )
...
# Taken from https://github.com/liu115/gaussian-splatting/blob/main/dataset.py
def readScannetppInfo(rootdir):
    ...
    images_dir = os.path.join(rootdir, "undistorted_images")
    ...
    extrinsic_dict = {}
    for _, image in camera_extrinsic.items():
    ...
    pcd = BasicPointCloud(
        points=xyz, colors=rgb / 255.0, normals=np.zeros_like(xyz)
    )  # TODO do we need / 255.0?
    ...
    if len(frames) > MAX_NUM_IMAGES_PER_SCENE:
        ...
        sample_indices = np.linspace(
            0, len(frames) - 1, MAX_NUM_IMAGES_PER_SCENE, dtype=np.int32
        )
    ...
    if "test_frames" in transforms:
        ...
        print("Found no test frames, subsampling to obtain them.")
        sample_indices = np.linspace(0, len(frames) - 1, 10, dtype=np.int32)
        test_frames = [frames[idx] for idx in sample_indices]
        frames = [
            frame for idx, frame in enumerate(frames) if idx not in sample_indices
        ]

    num_train_frames = len(frames)
    print(f"Loading {len(frames) + len(test_frames)} frames")
    for idx, frame in enumerate(frames + test_frames):
        ...
        try:
            ...
            # image does not exist, most likely evaluation image we replace with black to retain functionality
