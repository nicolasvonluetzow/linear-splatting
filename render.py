...
from linear_renderer import LinearModel, render
...
def render_set(
    model_path, name, iteration, views, linears, pipeline, background, kernel_size
):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        rendering = render(
            view, linears, pipeline, background, kernel_size=kernel_size
        )["render"]
        ...
        torchvision.utils.save_image(
            rendering, os.path.join(render_path, view.image_name + ".png")
        )
        torchvision.utils.save_image(
...
def render_sets(
    ...
    with torch.no_grad():
        ...
        scene = Scene(dataset, linears, load_iteration=iteration, shuffle=False)
